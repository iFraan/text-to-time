...
const timePoints = {
	...
	ahora: options => new DateManipulator(options.now, options.timeZone).getTime(), 
	'mañana': options => new DateManipulator(options.now, options.timeZone).add({days: 1}).getTime(),
	ayer: options => new DateManipulator(options.now, options.timeZone).subtract({days: 1}).getTime(),
	hoy: options => new DateManipulator(options.now, options.timeZone).getTime(),
}
...
function transformDate(expression, index, options) {
	...
	if (regex.test(e)) {
		...
		let parsedDate = formatUtils.parseNormalizedDate(e);
		parsedDate.months -= 1;
		let dateManipulator = new DateManipulator(parsedDate, options.timeZone);
		if(!parsedDate.years) {
			dateManipulator.setCurrentYear();
		}
		let value = dateManipulator.getTime();
...
function evaluate(expression, options, callback) {
	...
	if (stack.length == 1) {
		...
		if (result instanceof TimePoint) {
			return callback(undefined, result.value);
		}
	}
